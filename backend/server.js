...
if (!fs.existsSync(path.join(distPath, 'index.html'))) {
  console.warn('⚠️ WARNING: frontend/dist/index.html not found. Backend will not serve frontend.');
}
...
app.get('*', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.status(200).send('Backend is running. Frontend build missing. Please run "npm run build" in the root.');
  }
});
